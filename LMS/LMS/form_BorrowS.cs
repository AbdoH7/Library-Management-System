...
namespace LMS
{
    public partial class form_BorrowS : Form
    {
        ...
        private void buttonSave_Click(object sender, EventArgs e)
        {
            ...
            {
                if (s_name == Global.student[j].name)
                {
                    check1 = 1;
                    break;
                }
            ...
            {
                int flag = 0;
                for (int z = 0; z < Global.student[j].c_borrow; z++)
                {
                    if (b_name == Global.student[j].borrow[z].Name)
                    {
                        flag = 1;
                        break;
                    }
                ...
                if (flag == 0)
                {
                    ...
                    else if (Global.book[i].access == 1)
                    {
                        MessageBox.Show("Sorry, this book is available for teachers only");
                    }
                    else
                    {
                        Global.student[j].borrow.Add(Global.book[i]);
                        Global.student[j].c_borrow++;
                        Global.book[i].borrow++;
                        Global.wallet += Global.book[i].rent_price;


                        con.Open();
                        Query = @"insert into Student_Borrow values('" + j + "','" + Global.book[i].Name + "','" + Global.book[i].Author_name + "','" + Global.book[i].Publication_date + "','" + Global.book[i].Publisher + "','" + Global.book[i].rent_price + "','" + Global.book[i].state + "','" + Global.book[i].access + "')";
                        cmd = new SqlCommand(Query, con);
                        cmd.ExecuteReader();
                        con.Close();

                        con.Open();
                        Query = "update Students_Informations set no_Borrow='"+ Global.student[j].c_borrow +"' where Id= '"+j+"';";
                        cmd = new SqlCommand(Query, con);
                        cmd.ExecuteReader();
                        con.Close();
